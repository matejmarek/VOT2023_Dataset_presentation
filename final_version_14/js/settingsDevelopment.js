...
export function createSettingsElement(i) {
    ...
    const element = videoAspectsList[i];
    const widthElement = element[1];
    const heightElement = element[2];
    const lenthElement = element[3];
    let properties = anchorList[0].replace("width", widthElement);
    properties = properties.replace("height", heightElement);
    properties = properties.replace("lenth", lenthElement);
    p.innerHTML = properties;
    textSettings.appendChild(p);

    p = createElementNS("http://www.w3.org/1999/xhtml", "p");
    currentClass = "paragraph settings " + videoNameList[i];
    p.innerHTML = anchorList[1];
    textSettings.appendChild(p);
    ...
    for (const item of arrayOfArrays) {
        ...
        a.textContent = anchorList.slice(2)[arrayOfArrays.indexOf(item)];
