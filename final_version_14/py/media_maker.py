...
#This is a option to create zip folder of sequence frames, but it is a huge file
...
def write_new_list(source_folder, output_list_folder):
    ...
    for child in list_vot_child:
        ...
        lenth = len(items_in_directory)
        aspects_list.append((child, height, width, lenth))

    list_vot_child = os.listdir(source_folder)
    javascript_code = f"export const videoNameList = {json.dumps(list_vot_child)}"
    javascript_code2 = f"export const videoAspectsList = {json.dumps(aspects_list)}"
    new_list_path = os.path.join(output_list_folder, "newList.js")

    with open(new_list_path, "w") as js_file:
        js_file.write(f"{javascript_code}\n")
        js_file.write(javascript_code2)
    print(f"\033[92mList written.\033[0m")
...
def make_mask(source_folder):
    ...
    print(f"\033[92mMasks done.\033[0m")
...
def create_folders(source_folder, output_folder):
    ...
    for child in list_vot_child:
        ...
        print(f"\033[92mCreated folder number{folder_number}\033[0m")
    ...
    for child in list_vot_child:
        ...
        for img_path in list_num_paths:
            try:
                ...
                print(f"\033[92mSkipping {img_path} as it's not an image.\033[0m")
        ...
        print(f"\033[92mCompressed video number{video_number}\033[0m")

def repaire_color(source_folder):
    list_vot_child = os.listdir(source_folder)
    for child in list_vot_child:
        combined_output_path_color = os.path.join(source_folder, child, "color/00000000masked.jpg")
        combined_output_path_last = os.path.join(source_folder, child, "color/zzmasked.jpg")
        os.remove(combined_output_path_color)
        os.remove(combined_output_path_last)

if __name__ == '__main__':
    source_folder = "C:/Users/machm/vot_init_dataset_mini"  # VOT dataset path with structure: MAIN -> SEQUENCES_FOLDERS(name of video) -> COLOR -> IMGS_BY_NUMBER and OTHER_FILES
    output_folder = "C:/Users/machm/VOT2023_Dataset_presentation-main/version_15"  # where do you want the result videos with compression
    ...
    output_list_folder = "C:/Users/machm/VOT2023_Dataset_presentation-main/version_15/js" #replaced it with a path to your ja section
    ...
    print(f"\033[92mDone successfully!\033[0m")
