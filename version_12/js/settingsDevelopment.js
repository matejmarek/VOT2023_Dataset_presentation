...
export function createSettingsElement(i) {
    ...
    p.textContent = anchorList[0];
    textSettings.appendChild(p);

    const arrayOfArrays = [videoPathList, sequenceList, innitFramePathList, innitMaskList];
    ...
    p = createElementNS("http://www.w3.org/1999/xhtml", "p");
    currentClass = "paragraph settings " + videoNameList[i];
    p.innerHTML = linkString; // Use innerHTML to interpret the HTML in linkString
    textSettings.appendChild(p);
